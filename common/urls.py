...
from . import views
from django.contrib.auth import views as auth_views        #회원가입의 권한에서 갖고오는 views

app_name = 'common'
# 세션 : 일정시간동안 같은 사용자로부터 들어오는 요청을 하나의 상태로 보고
#       그 상태를 일정하게 유지시키는 기술.(사이트 방문자가 사이트에 접속해 있는 상태)
